...
(function() {
	...
	function runInContext(context) {
		...
		function Event(type) {
			...
			return (event == null || event.constructor != Event) ? new Event(type) : _.assign(event, {
				'timeStamp': _.now()
			}, typeof type == 'string' ? {
				'type': type
			} : type);
		...
		function clock() {
			...
			clock = function(clone) {
				...
				var funcBody = deferred ? 'var d#=this,${fnArg}=d#,m#=d#.benchmark._original,f#=m#.fn,su#=m#.setup,td#=m#.teardown;' +
				...
				if (!compiled && !deferred && !isEmpty) {
					funcBody = (
							stringable || (decompilable && !clone.error) ? 'function f#(){${fn}\n}var r#,s#,m#=this,i#=m#.count' : 'var r#,s#,m#=this,f#=m#.fn,i#=m#.count'
						) +
